        ...
        dref.child("ChatList").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                try {
                    ...
                    ChatAdapter chatAdapter = new ChatAdapter(messageAttrs, getApplicationContext(), ChatActivity.this);
            ...
            public void onClick(View v) {
                try {
                    if (!editText.getText().toString().equals(null)) {
                        ...
                        if (uper.contains("MEDICINE")) {
                            dref.child("Users").child(userId).addValueEventListener(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    if (dataSnapshot.exists()) {
                                        String type = dataSnapshot.child("category").getValue().toString();
                                        if (type.equals("Doctor")) {
                                            String push = dref.child("MP").push().getKey();
                                            dref.child("MP").child(push).child("id").setValue(push);
                                            dref.child("MP").child(push).child("receiverId").setValue(chaterId);
                                            dref.child("MP").child(push).child("senderId").setValue(userId);
                                            dref.child("MP").child(push).child("date").setValue(date);
                                            dref.child("MP").child(push).child("message").setValue(uper);
                                        }
                                    }
                                }
